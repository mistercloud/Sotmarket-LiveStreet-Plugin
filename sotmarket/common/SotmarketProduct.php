...
class SotmarketProduct {
    ...
    public function getProducts( $aProductsIds, $sProductName = '', $iCnt = 1,  $sTemplate = 'sotmarket_block' , $sImageSize = 'default', $aCategories = array()){
        ...
        foreach( $aProductsInfo as $iProductId => $aProductInfo ){
            ...
            $sFullImgUrl = str_replace(dirname(__FILE__),'',$sFullImgUrl);
            $sFullImgUrl = '/plugins/sotmarket/common'.$sFullImgUrl;
        ...
        $aParams = array();
        $aParams['aProducts'] = $aProducts;
        $aParams['iColumnsCnt'] = 1;
        $sReturn = $this->getRenderedData( $aParams , $sTemplate);
    ...
    protected function getRenderedData( $aParams = array() , $sTemplate = 'sotmarket_block'){

        extract($aParams);

        ob_start();
        include(dirname(__FILE__) . '/../templates/'. $sTemplate .'.php');
